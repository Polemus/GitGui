...
Omnigit holds credentials that can read and write all of your source code. Reports about
...
**[Report a vulnerability →](https://github.com/Polemus/Omnigit/security/advisories/new)**
...
The parts of Omnigit that would actually cost you something if they were wrong:
...
- **Host manifests** — a manifest is data by design and must not be able to execute
  anything, exfiltrate a token to a host it doesn't belong to, or be used to make Omnigit
...
- **The GitHub OAuth client id is checked in.** `DefaultClientId` in `GitHubProvider` is
  Omnigit's own OAuth App on github.com. A client id names an application on the approval
  ...
  `OMNIGIT_GITHUB_CLIENT_ID`, which overrides the default everywhere.
- **The `0600` file fallback for credentials.** When no OS keychain is available, tokens
  ...
  and Omnigit reports it — the store sets `IsSecure = false` and the UI warns. A report
...
- **Vulnerabilities in dependencies** with no path through Omnigit to exploit them.
  Report those upstream. If there is a path through Omnigit, report it here.

## Supported versions

Only the latest release. Omnigit is pre-1.0 and there are no maintenance branches — fixes
