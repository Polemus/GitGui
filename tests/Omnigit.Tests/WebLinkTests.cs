...
public class WebLinkTests
{
    ...
    {
        var url = WebLinks.CommitUrl("https://github.com/Polemus/Omnigit.git", Sha);

        Assert.Equal($"https://github.com/Polemus/Omnigit/commit/{Sha}", url?.ToString());
    }

    [Fact]
    public void AnSshRemoteGivesTheSameLinkAsAnHttpsOne()
    {
        var https = WebLinks.CommitUrl("https://github.com/Polemus/Omnigit.git", Sha);
        var ssh = WebLinks.CommitUrl("git@github.com:Polemus/Omnigit.git", Sha);
