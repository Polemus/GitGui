...
public sealed class SecretToolCredentialStore : ICredentialStore
{
    ...
    public async Task<string?> GetAsync(string key)
    {
        var (exitCode, output) = await ProcessRunner.RunAsync(
            "secret-tool", ["lookup", "service", "gitgui", "account", key]);
    ...
    public async Task SetAsync(string key, string secret)
    {
        ...
        var (exitCode, _) = await ProcessRunner.RunAsync(
            "secret-tool",
            ["store", "--label", $"GitGui ({key})", "service", "gitgui", "account", key],
            stdin: secret);
    ...
    public async Task DeleteAsync(string key)
        => await ProcessRunner.RunAsync("secret-tool", ["clear", "service", "gitgui", "account", key]);
}
...
public sealed class MacKeychainCredentialStore : ICredentialStore
{
    ...
    {
        var (exitCode, output) = await ProcessRunner.RunAsync(
            "security", ["find-generic-password", "-a", key, "-s", "GitGui", "-w"]);
    ...
    public async Task SetAsync(string key, string secret)
    {
        ...
        var (exitCode, _) = await ProcessRunner.RunAsync(
            "security", ["add-generic-password", "-a", key, "-s", "GitGui", "-w", secret, "-U"]);
    ...
    public async Task DeleteAsync(string key)
        => await ProcessRunner.RunAsync("security", ["delete-generic-password", "-a", key, "-s", "GitGui"]);
}
...
public sealed class DpapiCredentialStore : ICredentialStore
{
    private readonly string _directory = Path.Combine(
        Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "GitGui", "credentials");
